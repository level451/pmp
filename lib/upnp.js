...
//        var request = client.request("GET", l.pathname, {"host": l.hostname});
        ...
        request.addListener('response', function (response) {
            ...
            response.addListener("end", function() {
                ...
                gateway.ipurl = ipurl;
                x =new Gateway(l.port, l.hostname, ipurl,gateway);
                x.getExternalIP(callback);
...
Gateway.prototype.getExternalIP = function(callback) {
    ...
    this._getSOAPResponse(s, "GetExternalIPAddress", function(err, xml) {
        ...
        {
           this.gatewayobject.externalIP =  xml.match(/<NewExternalIPAddress>(.+?)<\/NewExternalIPAddress>/i)[1]
            callback(null,this.gatewayobject
               );
